        ...
        if (transcript.status === 'completed') {
          ...
          if (transcript.utterances && transcript.utterances.length > 0) {
            ...
            if (transcript.words && transcript.words.length > 0) {
              const wordConfidences = transcript.words.map(w => w.confidence || 0);
              const avgWordConfidence = wordConfidences.reduce((a, b) => a + b, 0) / wordConfidences.length;
              console.log(`Average word confidence: ${(avgWordConfidence * 100).toFixed(1)}%`);

              const lowConfidenceWords = transcript.words.filter(w => (w.confidence || 0) < 0.7);
              if (lowConfidenceWords.length > 0) {
                ...
                console.log('Low confidence words:', lowConfidenceWords.map(w => `"${w.text}" (${(w.confidence * 100).toFixed(1)}%)`));
              }
            }
