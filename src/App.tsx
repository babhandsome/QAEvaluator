...
const App: React.FC = () => {
  ...
  const defaultEvaluationCriteria: EvaluationCriteria[] = [
    {
      ...
      description:
        "Agent adhered to greeting script, identified themselves, mentioned company name, and offered assistance",
      maxScore: 25,
      weight: 2.5,
      keywords: [
        "good morning",
    ...
    },
    {
      ...
      description:
        "Agent took ownership, asked pertinent questions, provided appropriate solutions, and confirmed resolution",
      maxScore: 30,
      weight: 3.0,
      keywords: [
        "sorry",
    ...
    },
    {
      ...
      name: "Call Closure",
      description:
        "Agent followed closure guidelines, asked for additional questions, and thanked customer",
      maxScore: 15,
      weight: 1.5,
  ...
  const handleRubricFileUpload = async (
    event: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = event.target.files?.[0];
    if (file) {
      if (
        ...
        try {
          ...
          alert(
            "Failed to parse rubric file. Please check the format and try again.",
          );
        } finally {
      ...
      const mockParsedCriteria: EvaluationCriteria[] = [
        {
          ...
          description:
            "Agent provides warm, professional greeting with identification",
          maxScore: 20,
          weight: 2.0,
        ...
        },
        {
          ...
          description:
            "Agent demonstrates empathy and understanding of customer situation",
          maxScore: 25,
          weight: 2.5,
        ...
        },
        {
          ...
          description:
            "Agent effectively resolves customer issue with clear steps",
          maxScore: 30,
          weight: 3.0,
        ...
        },
        {
          ...
          description:
            "Agent ensures customer satisfaction and provides proper closure",
          maxScore: 15,
          weight: 1.5,
  ...
  const transcribeWithAssemblyAI = async (audioBlob: Blob): Promise<string> => {
    try {
      ...
      if (!transcriptResponse.ok) {
        ...
        throw new Error(
          `Transcription request failed: ${transcriptResponse.status} - ${errorData}`,
        );
      }
  ...
  const pollForTranscription = async (
      ...
      try {
        ...
        if (transcript.status === "completed") {
          ...
          if (transcript.confidence) {
            const confidencePercentage = (transcript.confidence * 100).toFixed(
              1,
            );
            ...
            if (transcript.confidence < 0.85) {
              console.warn(
                `⚠️ Lower confidence detected (${confidencePercentage}%). Consider improving audio quality.`,
              );
            }
          }
          ...
          if (transcript.utterances && transcript.utterances.length > 0) {
            ...
            if (transcript.words && transcript.words.length > 0) {
              const wordConfidences = transcript.words.map(
                (w: any) => w.confidence || 0,
              );
              const avgWordConfidence =
                wordConfidences.reduce((a: number, b: number) => a + b, 0) /
                wordConfidences.length;
              console.log(
                `Average word confidence: ${(avgWordConfidence * 100).toFixed(1)}%`,
              );

              const lowConfidenceWords = transcript.words.filter(
                (w: any) => (w.confidence || 0) < 0.7,
              );
              if (lowConfidenceWords.length > 0) {
                console.warn(
                  `⚠️ ${lowConfidenceWords.length} words with low confidence detected. Consider manual review.`,
                );
                console.log(
                  "Low confidence words:",
                  lowConfidenceWords.map(
                    (w: any) =>
                      `"${w.text}" (${(w.confidence * 100).toFixed(1)}%)`,
                  ),
                );
              }
            }

            return formatTranscriptWithAgentCustomer(
    ...
    throw new Error(
      "Transcription timeout - please try again with a shorter audio file",
    );
  };

  const formatTranscriptWithAgentCustomer = (
    utterances: any[],
    words?: any[],
  ): string => {
    ...
    utterances.forEach((utterance) => {
      ...
      const wordCount = utterance.words
      ...
      if (utterance.confidence) {
        ...
        speakerAnalysis[speaker].avgConfidence =
          speakerAnalysis[speaker].totalConfidence /
          speakerAnalysis[speaker].confidenceCount;
      }
    ...
    console.log(
      `✅ Identified Agent: Speaker ${agentSpeaker} (confidence-weighted)`,
    );
    ...
    const avgConfidence =
      utterances.reduce((sum, u) => sum + (u.confidence || 0), 0) /
      totalUtterances;
  ...
  const analyzeTranscript = () => {
    ...
    setTimeout(() => {
      const scores: ScoreResult[] = evaluationCriteria.map((criteria) => {
        ...
        switch (criteria.id) {
          ...
          case "custom_greeting":
            ...
            feedback = generateGreetingFeedback(
              transcript,
              score,
              criteria.maxScore,
            );
          ...
          case "custom_resolution":
            ...
            feedback = generateProblemSolvingFeedback(
              transcript,
              score,
              criteria.maxScore,
            );
          ...
          case "custom_followup":
            ...
            feedback = generateClosureFeedback(
              transcript,
              score,
              criteria.maxScore,
            );
          ...
          case "custom_empathy":
            ...
            feedback = generateEmpathyFeedback(
              transcript,
              score,
              criteria.maxScore,
            );
      ...
      const maxPossibleScore = evaluationCriteria.reduce(
  ...
  const analyzeGreeting = (text: string): number => {
    ...
    if (
      firstAgentLine.includes("good morning") ||
      firstAgentLine.includes("good afternoon") ||
      firstAgentLine.includes("good evening") ||
      firstAgentLine.includes("hello")
    ) {
      score += 5;
    }

    if (
      firstAgentLine.includes("thank you for calling") ||
      firstAgentLine.includes("thanks for calling")
    ) {
      score += 8;
    }

    if (
      firstAgentLine.includes("my name is") ||
      firstAgentLine.includes("this is")
    ) {
      score += 5;
    }

    if (
      firstAgentLine.includes("how can i help") ||
      firstAgentLine.includes("how may i assist") ||
  ...
  const analyzeProblemSolving = (text: string, maxScore: number): number => {
    ...
    const hasEmpathy = agentLines.some((line) =>
      empathyIndicators.some((indicator) =>
    ...
    const providesSolutions = agentLines.some((line) =>
      solutionIndicators.some((indicator) =>
        line.toLowerCase().includes(indicator),
      ),
    );
  ...
  const analyzeClosure = (text: string, maxScore: number): number => {
    ...
    const asksForMore =
      lastPortion.toLowerCase().includes("anything else") ||
      agentLines.some((line) =>
        additionalQuestions.some((phrase) =>
    ...
    const thanksAtEnd =
      lastPortion.toLowerCase().includes("thank you") ||
  ...
  const performGenericAnalysis = (
    ...
    if (interactionRatio > 0.8 && interactionRatio < 3) {
      // Good balance
  ...
  const extractAgentLines = (text: string): string[] => {
    return text
      ...
      .map((line) => line.replace("Agent:", "").trim());
  };
  ...
  const generateGreetingFeedback = (
    text: string,
    score: number,
    maxScore: number,
  ): string => {
    ...
    if (percentage >= 80) {
      feedback =
        "Excellent professional greeting with most required elements present.";
    } else if (percentage >= 60) {
      feedback =
        "Good greeting with some professional elements, but could be enhanced.";
    } else {
      feedback =
        "Basic greeting present but missing several professional elements.";
    }
  ...
  const generateProblemSolvingFeedback = (
    text: string,
    score: number,
    maxScore: number,
  ): string => {
    ...
    if (percentage >= 80) {
      feedback =
        "Excellent problem-solving approach with comprehensive customer support.";
    } else if (percentage >= 60) {
      feedback =
        "Good problem-solving skills demonstrated with room for improvement.";
    } else {
      feedback =
        "Basic problem-solving approach - consider more structured methodology.";
    }

    // Add specific observations
    const hasEmpathy = agentLines.some((line) =>
      ["sorry", "understand", "apologize"].some((word) =>
        line.toLowerCase().includes(word),
      ),
    );
    const hasQuestions = agentLines.some((line) =>
      ["what", "when", "how"].some((word) => line.toLowerCase().includes(word)),
    );
    const hasSolutions = agentLines.some((line) =>
      ["solution", "fix", "resolve"].some((word) =>
        line.toLowerCase().includes(word),
      ),
    );
  ...
  const generateClosureFeedback = (
    ...
    if (percentage >= 80) {
      ...
      feedback =
        "Basic call closure - consider adding more comprehensive wrap-up.";
    }
  ...
  const generateEmpathyFeedback = (
    ...
    if (percentage >= 80) {
      feedback =
        "Excellent empathy and rapport building throughout the conversation.";
    } else if (percentage >= 60) {
      feedback = "Good empathy demonstrated with positive customer connection.";
    } else {
      feedback =
        "Basic empathy shown - consider more emotional connection with customer.";
    }

    const empathyCount = agentLines.filter((line) =>
      ["sorry", "understand", "apologize", "frustrating"].some((word) =>
        line.toLowerCase().includes(word),
      ),
    ).length;
          ...
          <div className="upload-area">
            ...
            {!rubricFile && !isLoadingRubric && (
              <div className="rubric-info">
                <p>
                  📋 Using default rubric ({evaluationCriteria.length} criteria)
                </p>
                <p className="info-text">
                  Upload an Excel file with custom scoring criteria to use your
                  own rubric.
                  <br />
                  <strong>Expected columns:</strong> Name, Description, Max
                  Score, Weight, Keywords, Category
                </p>
              </div>
            )}
          </div>
        ...
        {transcript && (
          <section className="analysis-section">
            ...
            <div className="analysis-info">
              <p>
                📊 Ready to analyze using{" "}
                <strong>{evaluationCriteria.length} criteria</strong>
              </p>
              <p>
                Total possible score:{" "}
                <strong>
                  {evaluationCriteria.reduce((sum, c) => sum + c.maxScore, 0)}{" "}
                  points
                </strong>
              </p>
            </div>
            ...
            {evaluationCriteria.length === 0 && (
              <p className="warning-text">
                ⚠️ Please upload a scoring rubric first
              </p>
            )}
          </section>
        )}
          ...
          <section className="results-section">
            ...
            <div className="score-summary">
              <div
                ...
                style={{ color: getScoreColor(analysis.percentage) }}
              >
                <span className="score-number">{analysis.percentage}%</span>
                <span className="score-grade">
                  Grade: {getGrade(analysis.percentage)}
                </span>
              </div>
              <p>
                {analysis.totalScore} / {analysis.maxPossibleScore} points
              </p>
              <p className="rubric-used">
                Using: {rubricFile ? rubricFile.name : "Default Rubric"}
              </p>
            </div>
            ...
            <div className="detailed-scores">
              ...
              {analysis.scores.map((scoreResult) => {
                const criteria = evaluationCriteria.find(
                  (c) => c.id === scoreResult.criteriaId,
                )!;
                const percentage = Math.round(
                  (scoreResult.score / criteria.maxScore) * 100,
                );
